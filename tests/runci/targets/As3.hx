...
class As3 {
	static public function run(args:Array<String>) {
		...
		if (commandSucceed("mxmlc", ["--version"])) {
			...
			runCommand("wget", [
				"-nv",
				'${apacheMirror}/flex/${flexVersion}/binaries/apache-flex-sdk-${flexVersion}-bin.tar.gz'
			], true);
			...
			runCommand("wget", [
				"-nv",
				'http://download.macromedia.com/get/flashplayer/updaters/${flashVersion[0]}/playerglobal${flashVersion[0]}_${flashVersion[1]}.swc',
				"-O",
				playerglobalswcFolder + "/11.1/playerglobal.swc"
			], true);
