...
class Des
{
    ...
    static var S_BOX:Array<Array<Array<Int>>> =
    [
     [ // S1
    ...
    public function init(key:Bytes, ?iv:Bytes):Void
    {
        ...
        if ( keyLength < 8 ) throw "Specified key is not a valid size for Des algorithm";   
        this.iv = iv;
        this.key = keygen(key);
    }
