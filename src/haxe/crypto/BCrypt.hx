...
class BCrypt
{
    ...
    public static inline var Revision2Y = "2y";

    private static inline var BCRYPT_SALT_LEN : Int = 16;
    private static inline var P_SZ : Int = 18;
    private static inline var S_SZ : Int = 1024;
    private static inline var NUM_ROUNDS = 16;
    ...
    private static var KS:Array<Int> = [
        ...
        0xAD6EA6B0, 0x49A7DF7D, 0x9CEE60B8, 0x8FEDB266, 0xECAA8C71, 0x699A17FF,
        ...
        0x3D28F89E, 0xF16DFF20, 0x153E21E7, 0x8FB03D4A, 0xE6E39F2B, 0xDB83ADF7, 0xE93D5A68, 0x948140F7, 0xF64C261C, 0x94692934,
        ...
        0x362ABFCE, 0xDDC6C837, 0xD79A3234, 0x92638212, 0x670EFA8E, 0x406000E0, 0x3A39CE37, 0xD3FAF5CF, 0xABC27737, 0x5AC52D1B,
    ...
    private function hashPassword(password:String, salt:String):String
    {
        ...
        if (salt.length < 16) throw "Salt length is too short";
        if (salt.charAt(0) != '$' || salt.charAt(1) != '2') throw "Invalid salt version";
        if (salt.charAt(2) == '$') {
            offset = 3;
        } else {
            minor = salt.charAt(2);
            if ( (minor != 'y' && minor != 'x' && minor != 'a' && minor != 'b') || (salt.charAt(3) != '$') ) throw "Invalid salt revision";
             offset = 4;
        }
        if (salt.charCodeAt(offset + 2) > 36) throw "Missing salt rounds";
        var rounds =  Std.parseInt(salt.substr(offset,2));
        var realSalt  = salt.substr(offset+3,22);
        var isMinor = (minor != null); // .charCodeAt(0) >= 97
        var hashedPassword : String = "$2"+((isMinor)?minor:"")+"$";
        hashedPassword += ((rounds < 10)?"0":"") + rounds + "$";
        var saltBytes = decode_base64(realSalt);
        var passwordBytes = Bytes.ofString(password + (isMinor?String.fromCharCode(0):""));
        var hashedBytes =  cryptRaw(passwordBytes,saltBytes, rounds);
        if ( hashedBytes == null ) return null;
        var hashed = encode_base64(hashedBytes);
        var saltString = encode_base64(saltBytes);
        return ( hashedPassword + saltString + hashed );
    }

    public static function generateSalt(rounds:Int=10, saltRevision:String = BCrypt.Revision2B):String
    {
        if (rounds < 4 || rounds > 31) throw "Salt rounds should be between 4 and 31";
        var rng = Bytes.alloc(BCRYPT_SALT_LEN);
        for( i in 0...BCRYPT_SALT_LEN) {
            rng.set(i, (Std.random(256)-128) );
        }
        var salt =  "$"+saltRevision+"$" + ((rounds < 10) ? "0" : "") + rounds + "$";
        salt += encode_base64(rng);

        return salt;
    }

    public static function verify(plaintext:String, hash:String ):Bool
    {
        var bcrypt = new BCrypt();
        ...
        for(i in 0...candidate.length) {
            b  |= hash.charCodeAt(i) ^ candidate.charCodeAt(i);
        }
        return b==0;
    }

    private function cryptRaw(passwordBytes:Bytes, saltBytes:Bytes, numberOfRounds:Int):Bytes
    {
        ...
        for(i in 0...clen) 
        {
            b.set(j++,(cipherText[i] >>> 24) & 0xff);
            b.set(j++,(cipherText[i] >>> 16) & 0xff);
    ...
    private static function extractWord(data:Bytes):Int
    {
        var word : Int = 0;
        for(i in 0...4) {
            word = (word << 8) | (data.get(woffset) & 0xff);
            woffset = (woffset + 1) % data.length;
        }
        return word;
    }
    ...
    private function enhanceKeySchedule(password : Bytes, salt:Bytes):Void
    {
        ...
        woffset = 0;
        while(i < P_SZ) {
            lr[0] ^= extractWord(salt);
            lr[1] ^= extractWord(salt);
            encipher(lr, 0);
            pBox[i] = lr[0];
            pBox[i+1] = lr[1];
            i += 2;
    ...
    private static function encipher(block:Array<Int>, offset:Int):Void
    {
        ...
        xl ^= pBox[0];
        var i = 0;
        while (i <= (NUM_ROUNDS-2) ) {
            i += 1;
            xr ^= f(xl) ^ pBox[i];
            i += 1;
            xl ^= f(xr) ^ pBox[i];
        }
        block[offset] = xr ^ pBox[NUM_ROUNDS + 1];
        block[offset + 1] = xl;
    }

    private static function encode_base64(bytes:Bytes):String
    {
        return new BaseCode(BYTES).encodeBytes(bytes).toString();
    }

    private static function decode_base64(str:String):Bytes
    {
