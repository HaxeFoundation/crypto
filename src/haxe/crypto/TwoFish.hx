...
class TwoFish {
	...
	private function setKey(key:Bytes):Void {
		...
		if (keyLength != 16 && keyLength != 24 && keyLength != 32)
			throw "Key size should be 128, 192 or 256 bits";
	...
	private function lfsr2(x:Int):Int {
		return ((x >> 2) ^ ((x & 0x02) > 0 ? MDS_GF_FDBK_2 : 0) ^ ((x & 0x01) > 0 ? MDS_GF_FDBK_4 : 0));
	}
	...
	public function f32(x:Int, k32:Vector<Int>, keyLenght:Int):Int32 {
		...
		{
			x = mdsMatrix[0][Q0[Q0[xb0] ^ b0(k32[1])] ^ b0(k32[0])] ^ mdsMatrix[1][Q0[Q1[xb1] ^ b1(k32[1])] ^ b1(k32[0])] ^ mdsMatrix[2][Q1[Q0[xb2] ^ b2(k32[1])] ^ b2(k32[0])] ^ mdsMatrix[3][Q1[Q1[xb3] ^ b3(k32[1])] ^ b3(k32[0])];
		}
		return x;
	}
